...
window.sendData = async function sendData({url='https://script.google.com/macros/s/AKfycbzoho4vFdv5J2e06WGnIOhRW5O0VPAMeMRK9t55oyiGXMJo8Z0OlSC-hIS2Ju30le3xUA/exec', username, email, course, question, value}) {
   ...
   // NOTE: - Make sure to add new tabs to the training sheet (See Confluence Page for Link to Google Sheet)
...
<!---
NOTE: Each LiaScript tutorial is organized around "headers". A Header 1 (#), defines a primary section of a LiaScript tutorial. To add subsections (which appear as nested content in the LiaScript navigation bar), use appropraite subheaders. Example: 
# Primary Section 1
< Some Content >
...
<!---
NOTE: The following set of scripts will embed a form that gathers each user's RC credentials (if they have them). This should be included at the start of every training. 
--->
...
<script input="submit" default="Submit" style="display:block; text-align:center;"  >
  if(user_email.endsWith(".edu") || user_email.endsWith(".gov")){
    ...
    sendData({username: user_name, email: user_email, course:"TRAINING_NAME_SCORES", question:"START", value: currentDate.toLocaleString()})
    "Information Saved"
  }
  else{
...
<script hidden>
 let currentDate = new Date();
  sendData({username: user_name, email: user_email, course:"TRAINING_NAME_VISITS", question:"SECTION_NAME", value: currentDate.toLocaleString()})
  "LIA: wait"
</script>
...
<!--- 
 NOTE: Knowledge Check's serve as formative assessments that test a user's understanding of the covered material. 
   ...
   For more information and examples of LiaScript Quiz Questions, see their online documentation
   https://liascript.github.io/course/?https://raw.githubusercontent.com/liaScript/docs/master/README.md#1 

   Also, the online docs are themselves a LiaScript Tutorial. To learn more about quizzes, you should find a "Quizzes" section in
   the far left navigation bar. You may also want to change the style from "presentation" to "textbook" for easier reading. 
--->
...
//NOTE: - this question is designed to only record when a user submits a correct answer. This helps to reduce the number of calls to the online tracking sheet. 
if(check == 2){
  let currentDate = new Date();
  sendData({username: user_name, email: user_email, course:"TRAINING_NAME_SCORES", question:"UNIQUE_QUESTION_NAME", value: currentDate.toLocaleString()})
  send.lia("true")
} else { send.lia("")}
...
## Conclusion
<!---
 NOTE: Every tutorial should end with a quick overview that summarizes the training, points to additional CURC resources, and a closing survey.
--->
...
<script hidden>
 let currentDate = new Date();
  sendData({username: user_name, email: user_email, course:"TRAINING_NAME_VISITS", question:"CONCLUSION", value: currentDate.toLocaleString()})
  "LIA: wait"
</script>
...
<script>
  ...
  for (const [key, value] of Object.entries(choices)) {
    if(value === 1){
      sendData({username: user_name, email: user_email, course:"TRAINING_NAME_SCORES", question:"REVIEW_SCORE", value: key })
...
<script>
  ...
  sendData({username: user_name, email: user_email, course:"TRAINING_NAME_SCORES", question:"COMMENTS", value: feedback})
  send.lia("Feedback Sent", [], false)
  "LIA: wait"
</script>
